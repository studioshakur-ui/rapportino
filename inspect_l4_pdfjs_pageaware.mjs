...
function groupByRow(items, yTolerance = 4) {
  ...
  for (const item of sorted) {
    if (lastY === null || (lastPage === item.page && Math.abs(item.y - lastY) <= yTolerance)) {
      currentRow.push(item); lastY = item.y; lastPage = item.page;
    } else {
      if (currentRow.length > 0) rows.push(currentRow);
      currentRow = [item]; lastY = item.y; lastPage = item.page;
    }
  }
  if (currentRow.length > 0) rows.push(currentRow);
...
for (const row of rows.slice(0, 18)) console.log(row.map(i => `[p${i.page} ${i.x}:${i.str}]`).join(' '));
