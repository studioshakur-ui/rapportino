...
inca_rows as (
  select
    c.inca_data_posa::date as posa_date,
  ...
  where c.inca_data_posa is not null
),
...
select jsonb_build_object(
  ...
  'drilldown', jsonb_build_object(
    'available', true,
    'note', 'Courbe basée sur INCA (situazione + progress_percent) avec date posa INCA. Rapportino exclu.'
  )
);
$$;
