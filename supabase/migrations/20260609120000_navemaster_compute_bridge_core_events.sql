-- M1 — NAVEMASTER × CORE COMMAND : pont preuve terrain
--
-- compute_run_v2 compte désormais la vérité terrain du cockpit
-- (CABLE_POSATO + FIELD_VERIFIED positif dans core_events) comme PREUVE DE
-- PRÉSENCE, en plus de l'ancienne preuve rapportini (APPROVED_UFFICIO) qui
-- porte les mètres.
--
-- Principe clé : la présence terrain (sans mètres) clôt MISSING_IN_CORE et met
-- coverage=BOTH, SANS inventer de mètres → aucun faux METRI_MISMATCH.
-- Jointure sur code normalisé (les core_events n'ont pas inca_cavo_id fiable).
-- Gate d'exécution étendu au owner CORE COMMAND.
-- INCA reste en lecture seule. Idempotent (create or replace).
...
declare
  ...
  v_run_id uuid;
begin
  -- auth gate : owner CORE COMMAND OU ufficio/admin legacy
  ...
  -- ── PONT : vérité terrain cockpit comme preuve de présence (sans mètres) ──
  -- CABLE_POSATO (posé, remonté Telegram) + FIELD_VERIFIED positif (ou legacy
  -- sans status). Fenêtre alignée sur approved_from/to via occurred_at.
  ...
  select
    ...
    c.codice,
    -- status precedence: E > B > P(mètres OU présence terrain) > R > L > baseline
    coalesce(
      case
    ...
    -- last_proof_at = la plus récente entre rapportino et preuve cockpit
    greatest(pr.last_proof_at, cp.core_last_proof_at) as last_proof_at,
    pr.last_rapportino_id,

    -- coverage BOTH si preuve mètres OU preuve présence terrain
    case
  ...
  -- Alerts: EXTRA_IN_CORE (rapportini approuvés hors baseline) — inchangé
  ...
  -- Alerts: EXTRA_IN_CORE (preuve cockpit POSATO/FIELD_VERIFIED hors baseline)
  insert into public.navemaster_alerts (run_id, ship_id, costr, commessa, codice, codice_norm, type, severity, evidence)
  select
  ...
  where not exists (
    ...
    where c.inca_file_id = v_inca_file_id
      and upper(replace(replace(trim(c.codice), ' ', ''), '*', '')) = cp.codice_key
  );

  -- Alerts: DUPLICATE_IN_INCA — inchangé
  with dup as (
    select
      replace(replace(trim(c.codice), ' ', ''), '*', '') as codice_norm,
      count(*) as cnt,
  ...
  -- Alerts: STATUS_CONFLICT (E/R/L avec preuve mètres OU présence terrain ; B inactif)
  with latest_evt as (
    select distinct on (e.codice_norm)
      e.codice_norm, e.event_type, e.blocco_locale_id, e.event_at,
  ...
  -- Alerts: METRI_MISMATCH — inchangé
  insert into public.navemaster_alerts (run_id, ship_id, costr, commessa, codice, codice_norm, type, severity, evidence)
  select
    v_run_id, p_ship_id, v_costr, v_commessa, s.codice, s.codice_norm,
  ...
  -- Alerts: BLOCKED_IMPACT — inchangé
  with latest_evt as (
    select distinct on (e.codice_norm)
  ...
  -- verdict + drivers (inchangé : counts depuis alerts/state_rows)
...
comment on function public.navemaster_compute_run_v2(uuid,uuid,date,date,boolean)
  is 'NAVEMASTER V2 run: réconcilie le baseline INCA contre la preuve terrain = rapportini APPROVED_UFFICIO (mètres) + core_events CABLE_POSATO/FIELD_VERIFIED (présence). INCA read-only.';
