...
select jsonb_build_object(
  'meta', jsonb_build_object(
    ...
    'rules', jsonb_build_object(
      'stesura_giorno_includes', jsonb_build_array('STESURA'),
      ...
      'progress_pct',
        case
          when (select metri_ref_total from inca_scope) > 0
            then round(((select stesura_cum_m from headline) / (select metri_ref_total from inca_scope) * 100.0)::numeric, 4)
          else null
        end
    )
  ),
  ...
  'drilldown', jsonb_build_object(
    'available', true,
    'note', 'La funzione applica STESURA come posa (RIPRESA esclusa). Fascettatura è esclusa.'
  )
);
$$;
