...
CREATE TABLE IF NOT EXISTS "public"."profiles" (
    ...
    "email" "text",
    "role" "public"."app_role" DEFAULT 'CAPO'::"public"."app_role",
    "display_name" "text",
    "default_costr" "text",
...
CREATE POLICY "audit_select_direzione" ON "public"."core_file_audit" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
...
CREATE POLICY "core_files_delete_direzione" ON "public"."core_files" FOR DELETE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."app_role" = 'DIREZIONE'::"text")))));



CREATE POLICY "core_files_insert" ON "public"."core_files" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."app_role" = ANY (ARRAY['CAPO'::"text", 'UFFICIO'::"text", 'MANAGER'::"text", 'DIREZIONE'::"text"])) AND (("core_files"."cantiere" = COALESCE("p"."default_costr", "core_files"."cantiere")) OR (("to_jsonb"("p".*) ? 'allowed_cantieri'::"text") AND ("core_files"."cantiere" = ANY (COALESCE("p"."allowed_cantieri", ARRAY[]::"text"[])))))))));



CREATE POLICY "core_files_select" ON "public"."core_files" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND (("p"."app_role" = 'DIREZIONE'::"text") OR ("core_files"."cantiere" = COALESCE("p"."default_costr", "core_files"."cantiere")) OR (("to_jsonb"("p".*) ? 'allowed_cantieri'::"text") AND ("core_files"."cantiere" = ANY (COALESCE("p"."allowed_cantieri", ARRAY[]::"text"[])))))))));



CREATE POLICY "core_files_update" ON "public"."core_files" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND (("p"."app_role" = 'DIREZIONE'::"text") OR ("core_files"."created_by" = "auth"."uid"())))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND (("p"."app_role" = 'DIREZIONE'::"text") OR ("core_files"."created_by" = "auth"."uid"()))))));
