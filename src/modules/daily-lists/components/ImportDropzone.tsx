...
export default function ImportDropzone({
  ...
  disabled = false,
  accept = ".pdf,.xlsx,.xls",
  title = "Importa lista PDF",
  description = "Trascina qui la lista giornaliera o tocca per scegliere un file.",
  badges = ["PDF", "XLSX", "L1/L2/L3"],
}: Props): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <span className="flex flex-wrap items-center justify-center gap-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-zinc-500">
        {badges.map((badge) => (
          <span key={badge} className="rounded-full border border-zinc-700 bg-zinc-950/70 px-2.5 py-1">
            {badge}
          </span>
        ))}
      </span>
