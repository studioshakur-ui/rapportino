...
export default function RapportinoTable({ rows, onRowChange, onRemoveRow }) {
  return (
    <div className="mt-3">
      {/* Mobile-first: allow horizontal scroll without breaking the paper */}
      <div className="w-full overflow-x-auto">
        <table className="rapportino-table text-[11px] w-full min-w-[860px]">
          <thead>
            <tr className="bg-slate-50">
              ...
              <th className="px-2 py-2 text-center w-[140px]">
                TEMPO<br /><span className="text-[10px] text-slate-500">(ORE)</span>
              </th>
              <th className="px-2 py-2 text-right w-[120px]">PREVISTO</th>
              <th className="px-2 py-2 text-right w-[120px]">
                PRODOTTO<br /><span className="text-[10px] text-slate-500">(MT)</span>
              </th>
              ...
              <tr key={r.id || idx} className="align-top" data-ot-wrap>
                ...
                <td className="px-2 py-2">
                  ...
                  <textarea
                    ...
                    value={r.operatori || ""}
                    onChange={(e) => onRowChange(idx, "operatori", e.target.value, e.target)}
                ...
                <td className="px-2 py-2 text-center">
                  ...
                  <textarea
                    ...
                    placeholder={"8\n8\n8\n..."}
                    value={r.tempo || ""}
                    onChange={(e) => onRowChange(idx, "tempo", e.target.value, e.target)}
                    onInput={(e) => adjustOperatorTempoHeights(e.target)}
                  />
                ...
                <td className="px-2 py-2 text-right">
                  <div className="no-print">
                    <input
                      ...
                      value={r.previsto || ""}
                      onChange={(e) => onRowChange(idx, "previsto", e.target.value)}
                    />
                  </div>
                  <div className="print-only">
                ...
                <td className="px-2 py-2 text-right">
                  <input
                    className="w-full text-right bg-transparent outline-none"
                    value={r.prodotto || ""}
                    onChange={(e) => onRowChange(idx, "prodotto", e.target.value)}
                  />
                </td>
      ...
      <div className="mt-2 text-[11px] text-slate-500 no-print">
        Operatore & Tempo: una riga = una persona (nessuno scroll interno, la riga si espande automaticamente).
      </div>
    </div>
  );
}
