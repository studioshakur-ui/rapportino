// src/navemaster/i18n/navemaster.en.ts
export const navemasterEN = {
  NM_TITLE: "NAVEMASTER",
  NM_SUBTITLE: "Cockpit · Alerts · INCA Diff",

  NM_ACCESS_DENIED_TITLE: "Access denied",
  NM_ACCESS_DENIED_BODY:
    "NAVEMASTER is restricted to UFFICIO / DIREZIONE / ADMIN. If you think this is a mistake, contact an admin.",

  NM_SHIP_LABEL: "Ship",
  NM_IMPORT_ACTIVE: "Active import",
  NM_IMPORT_NONE: "No active import",

  NM_VIEW_COCKPIT: "Cockpit",
  NM_VIEW_ALERTS: "Alerts",
  NM_VIEW_DIFF: "INCA Diff",

  NM_REFRESH: "Refresh",
  NM_IMPORT: "Import",
  NM_CLOSE: "Close",

  NM_KPI_TOTAL: "Total",
  NM_KPI_NAV_STATUS: "Navemaster status",
  NM_KPI_ALERTS: "Alerts",
  NM_KPI_DIFF: "Diff",

  NM_FILTER_SEARCH: "Search (MARCA CAVO)",
  NM_FILTER_STATUS: "Status",
  NM_FILTER_ZONA: "Zone",
  NM_FILTER_SEZIONE: "Section",
  NM_FILTER_ONLY_WITH_INCA: "Only with INCA",

  NM_EMPTY_TITLE: "No data",
  NM_EMPTY_BODY: "Select a ship and import a NAVEMASTER file, or adjust filters.",

  NM_TABLE_MARCA: "MARCA CAVO",
  NM_TABLE_NAV_STATUS: "NAV",
  NM_TABLE_INCA_STATUS: "INCA",
  NM_TABLE_SEZIONE: "Section",
  NM_TABLE_ZONA_DA: "From zone",
  NM_TABLE_ZONA_A: "To zone",
  NM_TABLE_UPDATED: "INCA updated",

  NM_DETAILS_TITLE: "Details",
  NM_DETAILS_META: "Metadata",
  NM_DETAILS_PAYLOAD: "Payload",

  NM_ALERTS_TITLE: "Alerts",
  NM_ALERTS_SEVERITY: "Severity",
  NM_ALERTS_RULE: "Rule",
  NM_ALERTS_CREATED: "Created",
  NM_ALERTS_META: "Meta",

  NM_DIFF_TITLE: "INCA Diff",
  NM_DIFF_PREV: "Previous",
  NM_DIFF_NEW: "New",
  NM_DIFF_MATCH: "Match",
};