...
export default function CapoPresencePage(): JSX.Element {
    ...
    async function load() {
      ...
      try {
        ...
        if (sErr) {
          ...
          if (mounted) {
            setShip(null);
            // keep existing warn if already set, otherwise set default
            setShipWarn((prev) =>
              prev
                ? prev
                : "Ship non visibile (RLS) o non presente. La presenza resta utilizzabile."
            );
          }
        } else {
          ...
          if (!s && mounted) {
            setShipWarn((prev) =>
              prev
                ? prev
                : "Ship non trovato o non visibile (RLS). La presenza resta utilizzabile."
            );
          }
        }
  ...
  const mustConfirmUnassignedProceed =
    assignedToday === false && !allowUnassignedProceed;
  ...
  const confirmPresence = async () => {
    ...
    try {
      ...
      if (msg.includes("row-level security") || msg.includes("rls")) {
        setErr(
          "Errore RLS: accesso negato. Se questo ship non è assegnato oggi, chiedi al Manager di aggiornare il piano o usa il selettore cantieri."
        );
        return;
      }
