...
export function toneByIndex(idx: unknown, isDark: boolean): string {
  ...
  if (!Number.isFinite(n)) return isDark ? "text-slate-300" : "text-slate-600";
  if (n >= 1.2) return isDark ? "text-emerald-200" : "text-emerald-700";
  if (n >= 1.0) return isDark ? "text-sky-200" : "text-sky-700";
...
export function pickAccent(idx: unknown): string {
  const n = typeof idx === "number" ? idx : Number(idx);
  if (!Number.isFinite(n)) return "bg-slate-400/25";
  if (n >= 1.2) return "bg-emerald-400/30";
  if (n >= 1.0) return "bg-sky-400/30";
